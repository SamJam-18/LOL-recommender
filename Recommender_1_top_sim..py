...
def sort(sim_data):
    ...
    for nums, row in enumerate(similarity):
        #order the similarity for each row to get most similary users first
        max_ = (row).argsort()
        max_ =  max_[::-1]
        ...
        for num in max_:
            #stop the loop if we have champions
            if len(champ_rec) < 6:     
                champ_rec = []    

                # go through and append the champions recommended
                user = sim_data.iloc[num]
                champs = user.where(user == 1)
                champs = champs.dropna()
                for x in champs.index:
                    if x in champ_rec or x in drop_champ:
                        continue
                    else:
                        champ_rec.append(x)
            else:
        ...
        new_row = {'User Name': (sim_data.iloc[nums].name), 'Champs' : champ_rec[:5]}
